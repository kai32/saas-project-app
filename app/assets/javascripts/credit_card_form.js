...
function GetURLParameter(sParam){
  ...
  for (var i = 0; i < sURLVariables.length; i++) {
    ...
    if (sParameterName[0] == sParam) {
      return sParameterName[1]; //will have error if the url is ?something=   where there is a blank after '='
    }
  }
};
...
// function to handle the submit of the form and intercept the default event
  submitHandler = function(event){
    ...
    return false; //this will stop the form from being submitted normally(the post part)
  };
...
// handle event of plan drop down changing
  var handlePlanChange = function(plan_type, form){
    ...
    if(plan_type === 'premium'){
      ...
      $form.off('submit'); //off first, cus can only set once, so remove existing one if any just to be sure
