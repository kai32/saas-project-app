    ...
    if !::Milia.use_recaptcha  ||  verify_recaptcha
      ...
      Tenant.transaction  do 
        ...
        if @tenant.errors.empty?   # tenant created
          ...
          end # end if premium block
        else
          resource.valid?
          log_action( "tenant create failed", @tenant )
          render :new and return # if method executes beyond here, probably will cause error
        end # if .. then .. else no tenant errors
        
        if flash[:error].blank? || flash[:error].empty? # payment successful 
        # i think got bug. if tenant got error, it will log the error but flash msg not set, so this if block is still true
          ...
          render :new and return
        end # end if .. else ... no payment errors
